        ...
        <div class="builder-row grid grid-cols-2 gap-2 w-full relative" v-if="inputs.length >= 1">
            <template v-for="(formInput, index) in inputs" :key="props.pushCounter">
                ...
                <svg v-if="inputs.length >= 1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    ...
                    class="w-5 h-5 bottom-7 text-purple-700 cursor-pointer add-same-line text-center hidden addRowIcon absolute -right-5 top-[69%] -translate-y-1/2 animate__animated animate__fadeIn animate__fastest"
                ...
                <svg v-if="inputs.length >= 1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke-width="1.5" stroke="currentColor"
                    class="w-5 h-5 text-purple-700 cursor-pointer pushNewBuilder transition-all text-center hidden addBuilderIcon absolute -bottom-5 left-1/2 -translate-x-1/2 animate__animated animate__fadeIn animate__fastest"
